...
def run_RF(DataPath, LabelsPath, CV_RDataPath, OutputDir, GeneOrderPath = "", NumGenes = 0):
    ...
    col = col - 1 
    test_idx_r = robjects.r['Test_Idx']
    test_ind = [np.array(test_idx_r[i]) for i in range(len(test_idx_r))]

    train_idx_r = robjects.r['Train_Idx']
    train_ind = [np.array(train_idx_r[i]) for i in range(len(train_idx_r))]
